...
t=6  # threads per job
jobs=1  # number of parallel samples
...
process_sample() {
    ...
    if [ ! -d "${meme_output}" ]; then
        meme-chip -seed 1 -oc "${meme_output}" -time 240 -ccut 100 -dna -order 2 -minw 5 -maxw 8 -db ${motif_db} -meme-mod zoops -meme-nmotifs 3 -meme-searchsize 100000 -streme-pvt 0.05 -streme-align center -streme-totallength 4000000 -centrimo-score 5.0 -centrimo-ethresh 10.0 ${fasta_file}
